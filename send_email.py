...
parser.add_argument(
    "--local", action="store_true", help="Use local OAuth authentication"
)
parser.add_argument("--test", action="store_true", help="Test mode")
parser.add_argument(
    "--user",
    default="carlo",
    help="User name, to be defined in secrets/user_queries.json",
)
...
def get_html_message(houses):
    ...
    rendered_html = template.render(
        houses=houses, api_key_geoapify=secrets["api_key_geoapify"]
    )

    rendered_html = re.sub(
        "exclude_from_email.*?end_exclude_from_email",
        "",
        rendered_html,
        flags=re.DOTALL,
    )
    rendered_html = re.sub(
        "exclude_from_email_2.*?end_exclude_from_email_2",
        "",
        rendered_html,
        flags=re.DOTALL,
    )

...
parser.add_argument(
    "--update",
    help="update price and link or geodata",
...
if what_to_update == "price_and_link":
    ...
    for house in all_houses:
        ...
        update_result = db.update_one(
            ...
            {"$set": {"link": new_link, "price": new_price}},
        )
...
if what_to_update == "geodata":
    ...
    for house in db_houses:
        if "place_id" not in house.get("geodata", {}).keys():
            ...
            update_result = db.update_one(
                {"link": house["link"]}, {"$set": {"geodata": geodata}}
            )
...
if what_to_update == "email_sent_default":
    ...
    update_result = db.update_many(
        {"email_sent": {"$exists": False}}, {"$set": {"email_sent": []}}
    )
...
if what_to_update == "fix_wrong_link":
    ...
    for house in houses:
        ...
        update_result = db.update_one(
            {"link": house["link"]}, {"$set": {"link": new_link}}
        )
        print(f"Updated {update_result.modified_count} house(s)")
